	...
	const Vector4D &axis3, const float &theta3)
{
	...
	for (auto &tri : o.plist)
	{
		...
		{
			vertexs.trans = matrix_applyToVector(vertexs.local_pos, Object);
...
void Transform_t::Object_to_World(Geom_Object &o)
{
	...
	for (auto &tri : o.plist)
	{
		...
		{
			tri.vlist[i].trans = matrix_applyToVector(tri.vlist[i].trans, World);
			o.vlist[tri.p[i]].world_trans = tri.vlist[i].world_trans = tri.vlist[i].trans;
			tri.vlist[i].self_trans = matrix_applyToVector(tri.vlist[i].self_trans, World);
...
void Transform_t::pipeline_object(Geom_Object & o, const Camera_UVN &camera)
{
	Object_to_World(o);
	//shading_calculate(o, light_list);
	back_cull(o, camera);
	frustum_cull(o, camera);
	World_to_Camera(o, camera);
	Camera_to_Perspective_to_Screen(o, camera);
}

void Transform_t::pipeline_renderlist(Render_List & render_list, const Camera_UVN &camera)
{
	for (auto &obj : render_list.object_list)
	{
		pipeline_object(obj, camera);
	}
}
